...
function phaseAgentSummary(phase: any): string {
    if (phase.mode === "single") return phase.agent;
    if (phase.mode === "parallel") {
        return `parallel (${phase.tasks.length}: ${phase.tasks.map((t: any) => t.agent).join(", ")})`;
    }
    if (phase.mode === "chain") {
        return `chain (${phase.tasks.length}: ${phase.tasks.map((s: any) => s.agent).join(" → ")})`;
    }
    return "?";
}
...
export function renderSubagentToolCall(args: Static<typeof SubagentParams>, theme: any): Text {
    ...
    if (args.hybrid && args.hybrid.length > 0) {
        const phaseDescs = args.hybrid.map((p: any) => phaseAgentSummary(p)).join(" → ");
        return new Text(
            `${theme.fg("toolTitle", theme.bold("subagent"))} ${theme.fg("accent", "hybrid")} ${theme.fg("dim", `(${args.hybrid.length} phases: ${phaseDescs})`)}${scopeText}`,
            0,
            0,
        );
    }
...
export function renderSubagentToolResult(result: any, options: any, theme: any): Container | Text {
    ...
    if (details.mode === "hybrid") {
        // Hybrid summary header — show phase breakdown with per-phase stats
        const totalOk = details.results.filter((r) => r.exitCode === 0).length;
        const totalFailed = details.results.filter((r) => r.exitCode !== 0).length;
        const totalRunning = details.results.filter((r) => r.progress?.status === "running").length;
        const totalIcon =
            totalRunning > 0
                ? theme.fg("warning", "󱦟")
                : totalFailed === 0
        ...
        c.addChild(
            new Text(
                truncLine(
                    `${totalIcon} ${theme.fg("toolTitle", theme.bold("hybrid"))} ${totalOk}/${details.results.length} agents · ${totalRunning > 0 ? `${totalRunning} running · ` : ""}${formatTokens(totalTokens)} tok · ${formatDuration(totalDuration)}`,
                    w,
                ),
                0,
                0,
        ...
        const totalIcon = failed === 0 ? theme.fg("success", "") : theme.fg("error", "");
        ...
        for (let i = 0; i < details.results.length; i++) {
            ...
            c.addChild(
                new Text(
                    truncLine(
                        `${stepIcon} ${theme.fg("accent", `Step ${r.step ?? i + 1}: ${r.agent}`)}`,
                        w,
                    ),
                    0,
                    0,
