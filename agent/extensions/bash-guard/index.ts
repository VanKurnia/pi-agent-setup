...
export function hasFlag(args: string[], flag: string): boolean {
	return args.includes(flag) || args.some((a) => a.startsWith("-") && !a.startsWith("--") && a.length > 2 && flag.length === 2 && a.slice(1).includes(flag[1]));
}
...
export function analyzeSegment(seg: Token[]): Risk | null {
	...
	if (cmd === "rm" || cmd === "rmdir" || cmd === "unlink") {
		...
		if (rest.some((a) => a.startsWith("-") && a.slice(1).includes("f"))) reasons.push("forced delete (-f)");
