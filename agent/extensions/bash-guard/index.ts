...
export function analyzeSegment(seg: Token[]): Risk | null {
    ...
    if (cmd === "rm" || cmd === "rmdir" || cmd === "unlink") {
        ...
        if (rest.some((a) => a.startsWith("-") && a.slice(1).includes("f")))
...
async function promptRunOrAbort(ctx: any, command: string, risk: Risk): Promise<"run" | "abort"> {
    ...
    const choice = await (ctx.ui.custom as any)(
        (tui: any, theme: any, _kb: any, done: (result: "run" | "abort") => void) => {
            const container = new Container();
            container.addChild(new DynamicBorder((s: string) => theme.fg("warning", s)));
            container.addChild(
                new Text(
                    theme.fg("warning", theme.bold("Potentially destructive bash command")),
                    1,
                    0,
                ),
            );
...
// Helper: are we in a subagent session? Reads the depth field from subagents.json,
// which the subagent extension manages as a runtime counter persisted to disk.
// This avoids relying on process.env for in-process subagents.
function isSubagent(): boolean {
    try {
        const sp = join(getAgentDir(), "subagents.json");
        const raw = JSON.parse(readFileSync(sp, "utf-8"));
        return (raw.depth ?? 0) >= 1;
    } catch {
        return false;
    }
}
...
function getHeadlessBlocked(config?: BashGuardConfig): Array<{ pattern: RegExp; reason: string }> {
    if (config?.headlessBlocked) {
        return config.headlessBlocked.map((item) => ({
...
export default function (pi: ExtensionAPI) {
    ...
    // Subagent mode is handled dynamically inside the tool_call handler below.
    ...
    pi.on("tool_call", async (event, ctx) => {
        ...
        // Dynamic subagent check: headless hard-block only, no prompting.
        // Subagent sessions share the same process, so the env var is the only reliable signal.
        if (isSubagent()) {
            const blocked = getHeadlessBlocked(config);
            ...
            // Not blocked → allow without further scrutiny
            return;
        }
        ...
        if (config.customRisks?.highRiskCommands?.some((cmd) => command.startsWith(cmd))) {
            const risk: Risk = {
                severity: "high",
                reasons: [`${command} is on the high-risk command list`],
            };
            ...
            return {
                block: true,
                reason: "Blocked by bash-guard: command is on the high-risk list. Ask the user for confirmation or propose a safer alternative.",
            };
        }
