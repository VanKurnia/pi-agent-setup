...
function computeBreakdown(ctx: any): ContextBreakdown | null {
    ...
    addCat("user", "User Messages", userTokens, (_th, t) => ansi256Fg(75, t), ansi256Bg(75, "  ")); // Blue
    ...
    for (const [name, tokens] of sortedTools) {
        ...
        const colorCode =
            builtin?.colorCode ?? customToolColors[customColorIdx++ % customToolColors.length]!;
        const label = builtin?.label ?? `Tool: ${name}`;
        addCat(
            `tool:${name}`,
            label,
            tokens,
            (_th, t) => ansi256Fg(colorCode, t),
            ansi256Bg(colorCode, "  "),
        );
    }
    ...
    addCat("images", "Images", imageTokens, (_th, t) => ansi256Fg(219, t), ansi256Bg(219, "  ")); // Pink
