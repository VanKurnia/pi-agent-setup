...
function getPlanArtifactApi(): PlanArtifactApi | null {
    try {
        return getExtensionApi<PlanArtifactApi>("plan-artifact") ?? null;
    } catch {
        return null;
    }
}
...
export function restoreTools(pi: ExtensionAPI, toolsBeforeReadOnly?: string[]): void {
    const allToolNames = new Set(pi.getAllTools().map((tool) => tool.name));
    const toolNames = (toolsBeforeReadOnly ?? pi.getAllTools().map((tool) => tool.name)).filter(
        (toolName) => allToolNames.has(toolName),
    );
...
export default function readOnlyModeExtension(pi: ExtensionAPI) {
    ...
    function enablePlanMode(ctx: ExtensionContext): void {
        ...
        updateUi(pi, ctx, readOnlyEnabled, planModeEnabled, executionMode, todoItems);
        ctx.ui.notify(
            "Plan mode enabled. Write tools disabled; bash is restricted to read-only commands.",
        );
        persistState();
    }
    ...
    pi.registerCommand(PLAN_COMMAND_NAME, {
        description:
            "Toggle plan mode (read-only exploration with bash allowlist, plan extraction, and step tracking)",
        handler: async (args, ctx) => {
            const action = args.trim().toLowerCase();
...
- Propose your plan using the \`plan_artifact\` tool — it renders in the browser with section commenting, syntax highlighting, and mermaid diagrams.
- Do NOT attempt to make changes — the agent can only execute after the plan is accepted through plan_artifact.
- Once accepted, execution will proceed automatically with step tracking.`,
                    display: false,
                },
            };
        }
...
After completing a step, include a [DONE:n] tag in your response where n is the step number.`,
    ...
    pi.on("agent_end", async (event, ctx) => {
        ...
        // Check plan-artifact API first
        const paApi = getPlanArtifactApi();
        const paStatus = paApi?.getStatus();

        if (paStatus === "accepted") {
            // Plan was accepted via plan-artifact — auto-enter execution
            const planContent = paApi!.getPlanContent() || "";
        ...
        if (paStatus === "pending") {
            ctx.ui.notify(
                "Plan is pending review. Accept via /plan-artifact or the browser to execute.",
                "info",
            );
            return;
        }

        // Fall back to inline plan extraction from assistant message
