...
// Memoized accent palette + logo lines, keyed by accent ANSI string (palette)
// and accent ANSI + width (logo). Rebuilt only on theme change (or width
// change for logo); avoids per-render palette rebuilds on message-hot paths.
let cachedAccentAnsi: string | null = null;
let cachedPalette: Rgb[] | null = null;
const logoLinesCache = new Map<string, string[]>();

function getCachedPalette(theme: { getFgAnsi(name: string): string }): {
    palette: Rgb[];
    accentAnsi: string;
} {
    const accentAnsi = theme.getFgAnsi("accent");
    if (cachedPalette && cachedAccentAnsi === accentAnsi) {
        return { palette: cachedPalette, accentAnsi };
    }
    const accentRgb = resolveAccentRgb(theme);
    cachedPalette = buildAccentPalette(accentRgb);
    cachedAccentAnsi = accentAnsi;
    return { palette: cachedPalette, accentAnsi };
}

function getCachedLogoLines(width: number, palette: Rgb[], accentAnsi: string): string[] {
    const key = `${accentAnsi}::${width}`;
    const hit = logoLinesCache.get(key);
    if (hit) return hit;
    const lines = renderLogoLines(width, palette);
...
function renderHeaderLines(
    ...
    const { palette, accentAnsi } = getCachedPalette(theme);
    const logoLines = getCachedLogoLines(width, palette, accentAnsi);
...
export default function piStartupHeader(pi: ExtensionAPI) {
    pi.on("session_start", async (_event, ctx) => {
        ...
        ctx.ui.setHeader((_tui, theme) => ({
            ...
            // Drop memoized palette/logo so a theme change renders fresh.
            invalidate() {
                cachedAccentAnsi = null;
                cachedPalette = null;
