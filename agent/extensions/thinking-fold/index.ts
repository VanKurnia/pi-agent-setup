...
const THINKING_FOLD_SETTINGS_COMMAND = "thinking-fold-settings";
...
export default function (pi: ExtensionAPI) {
  ...
  const showResponding = (ctx: ExtensionContext) => {
    clearSummaryHold();
    stopItemTimer();
    if (!config.showCursorLabel) return;
  ...
  // The shared settings helper hardcodes `/99settings`. Give it a proxy so
  // this standalone extension exposes its settings under its own command.
  const settingsPi = new Proxy(pi, {
    get(target, property, receiver) {
      if (property === "registerCommand") {
        return (name: string, options: Parameters<ExtensionAPI["registerCommand"]>[1]) =>
          target.registerCommand(
            name === SHARED_SETTINGS_COMMAND ? THINKING_FOLD_SETTINGS_COMMAND : name,
            options,
          );
      }
  ...
  registerExtensionSettings(settingsPi, {
    ...
    settings: () => [
      ...
      },
      {
        id: "showCursorLabel",
        label: "Cursor label",
        description: "Show a working-message status while thinking; disable if another extension owns the working message",
        currentValue: String(config.showCursorLabel),
        values: ["true", "false"],
      },
    ],
    onChange: (id, value, ctx) => {
      ...
      } else if (id === "showCursorLabel") {
        applyConfig({ ...config, showCursorLabel: value === "true" }, ctx);
      }
    },
  });
  ...
  const clearStreamState = (ctx: ExtensionContext) => {
    ...
    summaryFirstVisibleAt = undefined;
    lastWorkingMessage = undefined;
    if (!config.showCursorLabel || ctx.mode !== "tui") return;
