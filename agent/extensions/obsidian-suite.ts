...
export default function (pi: ExtensionAPI) {
    ...
    function collectVaultContext(vp: string): string {
        ...
        parts.push(
            "## Resolution Order\n1. `search_graph` / `read_symbol` — cari code symbol di codebase saat vault note merujuk ke implementation code (via `Projects/` paths). Use `search_graph` first for symbol discovery, then `read_symbol` or `get_code_snippet` to read source.\n2. `resolve_pi_url` — follow any `pi://` URL in context (vault, workspace, health, skill, db). Don't re-resolve content already in context.\n3. `ffgrep` — fallback search across vault files",
        );
