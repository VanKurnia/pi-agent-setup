...
export function registerOperations(pi: ExtensionAPI) {
    ...
    pi.registerTool({
        ...
        promptGuidelines: ["Use for comparing branches or viewing commit diffs"],
        parameters: Type.Object({
            repo_path: Type.String({ description: "Path to Git repository" }),
    ...
    pi.registerTool({
        ...
        promptGuidelines: ["Stage files before git_commit", "Use file paths relative to repo root"],
        parameters: Type.Object({
            repo_path: Type.String({ description: "Path to Git repository" }),
                    ...
                    if (!resolvedFile.startsWith(resolvedRepo)) {
                        return {
                            content: [
                                {
                                    type: "text",
                                    text: `Error: Path traversal blocked. File '${file}' is outside the repository directory.`,
                                },
                            ],
                            isError: true,
                            details: {},
    ...
    pi.registerTool({
        ...
        promptGuidelines: ["Use to browse recent commits or check history in a date range"],
        parameters: Type.Object({
            repo_path: Type.String({ description: "Path to Git repository" }),
            max_count: Type.Optional(
                Type.Number({ description: "Maximum number of commits to show (default: 10)" }),
            ),
            start_timestamp: Type.Optional(
                Type.String({
                    description: "ISO 8601, relative (e.g. '2 weeks ago'), or absolute date",
                }),
            ),
    ...
    pi.registerTool({
        ...
        promptGuidelines: ["Creates a branch from an optional base branch"],
        parameters: Type.Object({
            repo_path: Type.String({ description: "Path to Git repository" }),
    ...
    pi.registerTool({
        ...
        promptGuidelines: ["Switches branches — use with caution as it changes working tree"],
        parameters: Type.Object({
            repo_path: Type.String({ description: "Path to Git repository" }),
    ...
    pi.registerTool({
        ...
        promptGuidelines: ["Use with a commit hash, branch name, or tag to inspect changes"],
        parameters: Type.Object({
            repo_path: Type.String({ description: "Path to Git repository" }),
    ...
    pi.registerTool({
        ...
        promptGuidelines: ["Use to see local, remote, or all branches"],
        parameters: Type.Object({
            repo_path: Type.String({ description: "Path to Git repository" }),
            branch_type: Type.String({ description: "List 'local', 'remote', or 'all' branches" }),
            contains: Type.Optional(
                Type.String({ description: "The commit SHA that branch should contain" }),
            ),
            not_contains: Type.Optional(
