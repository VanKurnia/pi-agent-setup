...
import {
    FOOTER_PREFIX_PRESETS,
    ICON_PRESETS,
    LABEL_PRESETS,
    WORKING_PREFIX_PRESETS,
} from "./presets";
...
async function pickNumberPreset(
    ...
    const options = [
        currentText,
        ...presets.map(String).filter((preset) => preset !== currentText),
        CUSTOM_OPTION,
    ];
...
async function pickPreset(
    ...
    if (choice === CUSTOM_OPTION)
        return (
            (await ctx.ui.input(
                title,
                current === RANDOM_VALUE || current === OFF_VALUE ? presets[0] : current,
            )) ?? current
        );
    return choice;
}
...
export async function openSettings(
    ...
    while (true) {
        ...
        if (choice.startsWith("Enabled:")) config.enabled = !config.enabled;
            ...
            config.label = await pickPreset(ctx, "Speed label", config.label, LABEL_PRESETS, true);
        else if (choice.startsWith("Footer status:")) config.footer = !config.footer;
        else if (choice.startsWith("Footer prefix:"))
            config.footerPrefix = await pickPreset(
                ctx,
                "Footer prefix",
                config.footerPrefix,
                FOOTER_PREFIX_PRESETS,
            ...
            config.workingPrefix = await pickPreset(
                ctx,
                "Working prefix",
            ...
            config.icon = await pickPreset(
                ctx,
                "Speed badge icon (first half plain, second half Nerd Font)",
                config.icon,
                ICON_PRESETS,
                false,
            );
        else if (choice.startsWith("Render interval:"))
            config.renderIntervalMs = await pickNumberPreset(
                ctx,
                "Render interval ms",
            ...
            const min = await pickNumberPreset(
                ctx,
                "RunCat min frame interval ms",
                config.minRuncatIntervalMs,
                [30, 50, 75, 100],
            );
            const max = await pickNumberPreset(
                ctx,
                "RunCat max frame interval ms",
                config.maxRuncatIntervalMs,
                [167, 200, 250, 300, 500],
            );
            if (min <= max) {
        ...
        else if (choice.startsWith("Persist session stats:"))
            config.persistStats = !config.persistStats;
