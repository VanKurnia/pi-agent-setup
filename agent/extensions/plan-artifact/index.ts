...
export default function (pi: ExtensionAPI) {
    ...
    registerExtensionApi("plan-artifact", {
        ...
        getStatus: () => server.getProposal()?.status ?? null,
        getPlanContent: () => server.getProposal()?.plan ?? null,
    });
        ...
        while (true) {
            ...
            if (proposal.status === "pending") {
                actionItems.push({
                    value: "__accept__",
                    label: "Accept plan",
                    description: "Mark as accepted and proceed",
                });
                actionItems.push({
                    value: "__reject__",
                    label: "Request changes",
                    description: "Request revisions with feedback",
                });
            }
            ...
            const picked = await ctx.ui.custom(
                (tui: any, tuiTheme: any, _kb: any, done: any) => {
                    ...
                    const statusText =
                        proposal.status === "accepted"
                            ...
                            : proposal.status === "revising"
                              ...
                              : "\u25CF Pending";
                    container.addChild(
                        new Text(tuiTheme.fg("accent", tuiTheme.bold("Plan Review")), 1, 0),
                    );
            ...
            if (picked === "__accept__") {
                const ok = await ctx.ui.confirm(
                    "Accept plan?",
                    "Mark as accepted and proceed with implementation?",
                );
                if (!ok) continue;
                proposal.status = "accepted";
                pi.sendUserMessage(
                    `**Plan accepted**: "${proposal.summary}". Proceed with implementation.`,
                    { deliverAs: "followUp" },
                );
                server.stop();
            ...
            if (picked === "__reject__") {
                ...
                proposal.status = "revising";
                pi.sendUserMessage(
                    `**Plan needs revision**: "${proposal.summary}"\n\n**Feedback:**\n${feedback}`,
                    { deliverAs: "followUp" },
                );
                server.stop();
            ...
            if (picked.startsWith("section:")) {
                ...
                await ctx.ui.custom(
                    (tui: any, tuiTheme: any, _kb: any, done: any) => {
                        const container = new Container();
                        container.addChild(
                            new DynamicBorder((s: string) => tuiTheme.fg("accent", s)),
                        );
                        container.addChild(
                            new Text(tuiTheme.fg("accent", tuiTheme.bold(section.title)), 1, 0),
                        );
                        ...
                        if (sectionMd.trim()) {
                            ...
                            container.addChild(
                                new Text(tuiTheme.fg("muted", "(no content)"), 1, 0),
                            );
                        }
    ...
    pi.registerTool({
        ...
        label: "Propose Plan",
        description:
            "Propose a markdown plan for user review. Starts a local web UI showing the rendered plan with inline commenting.",
        ...
        parameters: Type.Object({
            ...
            plan: Type.String({
                description:
                    "Full plan in markdown format. Use ## headings for major sections, ### for subsections.",
            }),
        }),
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            if (server.getProposal()?.status === "pending") {
                return {
                    content: [
                        {
                            type: "text",
                            text: `A plan is already pending at ${server.getUrl()}. Accept or reject it first.`,
                        },
                    ],
                    details: {},
                };
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Plan created \u2014 ${sections.length} section(s).\nPreview \u0026 review: ${url}\nRun /plan-artifact to open TUI review.`,
                    },
                ],
