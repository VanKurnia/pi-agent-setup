...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (lastLinkData?.file) {
      try {
        await fs.access(lastLinkData.file);
        linkedFile = lastLinkData.file;
        lastKnownContent = await fs.readFile(lastLinkData.file, "utf-8");
        ...
        // File doesn't exist or can't be accessed — skip restoration
      }
    }
  });
  ...
  pi.registerCommand("link-md", {
    ...
    handler: async (args, ctx) => {
      ...
      linkedFile = resolved;
      lastKnownContent = await fs.readFile(resolved, "utf-8");
      ...
      ctx.ui.notify(`Linked: ${resolved}`, "info");
    },
  });
  ...
  const handleSendDiff = async (_args: string, ctx: any) => {
    ...
    const current = await fs.readFile(linkedFile, "utf-8");
  ...
  pi.on("message_end", async (event, _ctx) => {
    ...
    try {
      ...
      await fs.writeFile(linkedFile, updated, "utf-8");
